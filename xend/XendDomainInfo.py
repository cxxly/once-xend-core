...
from xen.util import asserts, auxbin, mkdir, Netctl
...
from xen.xend.XendConfig import scrub_password, dev_name_to_number
from xen.xend.XendBootloader import bootloader, bootloader_tidy
from xen.xend.XendError import XendError, VmError, VDIError
from xen.xend.XendError import XEND_ERROR_VM_BAD_POWER_STATE, XEND_ERROR_OPERATION_NOT_ALLOWED
...
#frank
from xen.xend.XendCPUQoS import XendCPUQoS
from xen.xend.XendNetworkQoS import XendNetworkQoS
...
from XendLogging import log_dominfo, init
...
GUEST_DIR = '/opt/xen/performance/guest'
OVS_VSCTL = os.popen("which ovs-vsctl").read().strip()

xc = xen.lowlevel.xc.xc()
xoptions = XendOptions.instance()

init("/var/log/xen/dominfo.log", "DEBUG", log_dominfo)
log = log_dominfo

if getConfigVar('compute', 'VM', 'cpu_num_limit'):
    CPU_NUM_LIMIT = int(getConfigVar('compute', 'VM', 'cpu_num_limit'))
else:
    CPU_NUM_LIMIT = 256
    
if getConfigVar('compute', 'VM', 'mem_limit'):
    MEM_LIMIT = int(getConfigVar('compute', 'VM', 'mem_limit'))
else:
    MEM_LIMIT = 256 #GB
...
#    xeninfo['is_local_vm'] = False
    ...
    log.info("Recreating domain %d, UUID %s. name %s. at %s" %
             (domid, xeninfo['uuid'], xeninfo['name_label'], dompath))
    ...
    log.debug('before init: %s' % xeninfo['name_label'])
...
def restore(config):
    ...
    vm = XendDomainInfo(XendConfig.XendConfig(sxp_obj = config),
                        resume = True)
    XendDomain.instance()._managed_domain_register(vm)
    XendDomain.instance().managed_config_save(vm)
    try:
        vm.resume()
        return vm
    except:
...
#    @type state: enum(DOM_STATE_HALTED, DOM_STATE_RUNNING, ...)
    ...
    __device_delete_lock__ = threading.Lock()
...
#        log.debug('1...in XendDomainInfo init: %s' % self.info['name_label'])

        if self.info.is_hvm() and maxmem > memory:
            self.pod_enabled = True
        else:
            self.pod_enabled = False
#        log.debug('2...in XendDomainInfo init: %s' % self.info['name_label'])
        ...
        while self.vmpath == None:
            ...
            try:
                ...
                pass
#        log.debug('3...in XendDomainInfo init: %s' % self.info['name_label'])
...
#        log.debug('4...in XendDomainInfo init: %s' % self.info['name_label'])
...
#        log.debug('5...in XendDomainInfo init: %s' % self.info['name_label'])

        name = self._checkName(self.info['name_label'])
        log.debug("after _checkName: %s" % name)
        self.info['name_label'] = name

        self.metrics = XendVMMetrics(uuid.gen_regularUuid(), self)
        #frank
        self.cpu_qos = XendCPUQoS(uuid.gen_regularUuid(), self)
        #frank
        self.network_qos = XendNetworkQoS(uuid.gen_regularUuid(), self)
    
    
    def getXenInfo(self):
        return dict(self.info)      
        
    def dominfo_set_all_tag(self):
        log.debug('dominfo_set_all_tag')
        vif_refs = self.get_vifs()
        for vif_ref in vif_refs:
            tag = '-1'
            eth_num = '-1'
            try:
                other_config = self.info.get('other_config', {})
                device = XendDomain.instance().get_dev_property_by_uuid('vif', vif_ref, 'device')
                ...
                log.exception(exn) 
            if eth_num != '-1' and tag != '-1':
                self.set_tag(int(eth_num), tag)
                
       
    def dominfo_set_all_rate(self, type):
        log.debug('dominfo_set_all_rate, type: %s' % type)
        vif_refs = self.get_vifs() 
        for vif_ref in vif_refs:
            tag = '-1'
            eth_num = '-1'
            try:
                ...
                log.exception(exn) 
            if eth_num != '-1' and tag != '-1':
                self.set_rate(type, eth_num, tag)
    
    def dominfo_get_IO_rate_limit(self, type, io_unit):
        log.debug('dominfo get IO rate limit')
        rate = '-1'
        tag = '%s_%s_rate' % (type, io_unit)
        other_config = self.info.get('other_config', {})
        if other_config:
            rate = self.info['other_config'].get(tag, '-1')
            #log.debug('>>>>rate: %s' % rate)
        return rate
    
    def dominfo_set_IO_rate_limit(self, pid2num_list, type, value, io_unit): 
        try:
            value = int(value)
            if value >= 0:
                tag = '%s_%s_rate' % (type, io_unit)
                log.debug('Set vm(%s)  %s: %s  MBps' %(self.getName(), tag, value)) 
                XendIOController.set_VM_IO_rate_limit(pid2num_list, type, value, io_unit)
                
                self.info.setdefault('other_config',{})
                self.info['other_config'][tag] = value     
                #XendDomain.instance().managed_config_save(self)                                           
        except Exception, exn:
            log.debug(exn)
    
    def dominfo_VBD_get(self, vbd_ref, prop):
        return XendDomain.instance().get_dev_property_by_uuid(
            'vbd', vbd_ref, prop)
        
    def dominfo_get_VDI(self, ref):
    ...
    def get_system_vbd_location(self):
        retv = None
        dev_info = self.info.get('devices')
        for dev_uuid in dev_info:
            ...
            if dev_type.startswith('tap') or dev_type.startswith('vbd'):
                if dev_cfg['dev'].endswith('disk') and cmp(dev_cfg['mode'], 'w') == 0 \
                    and cmp(dev_cfg['bootable'], 1) == 0:
                    retv = dev_cfg['uname'].split(':')[-1]
#            log.debug("get_vdi_by_vm:")
            log.debug("VBD location: %s" % retv)
        return retv
    
    def _get_system_vbd_driver(self):
        retv = None
        dev_info = self.info.get('devices')
        for dev_uuid in dev_info:
            dev_type = dev_info[dev_uuid][0]
            ...
            if dev_type.startswith('tap') or dev_type.startswith('vbd'):
                if dev_cfg['dev'].endswith('disk') and cmp(dev_cfg['mode'], 'w') == 0 \
                    and cmp(dev_cfg['bootable'], 1) == 0:
                    retv = dev_cfg['uname'].split(':')[0]
                    break
#            log.debug("get_vdi_by_vm:")
            log.debug("VBD driver: %s" % retv)
        return retv
    
#     def dominfo_get_dev2path_list(self):
# #         log.debug('>>>>dominfo_get_dev2path_list')
#         dev2path_list = {}
#         vbd_refs = self.get_vbds()
#         log.debug(vbd_refs)
#         for vbd_ref in vbd_refs:
#             try:
#                 if self.dominfo_VBD_get(vbd_ref, 'type').lower() == 'disk':
#                     dev = self.dominfo_VBD_get(vbd_ref, 'device')
#                     location = self.dominfo_VBD_get(vbd_ref, 'uname')
...
#                 log.debug(exn)
#                 return {}
#         log.debug('VM_get_dev2path_list: %s ' % dev2path_list)
#         return dev2path_list
    
    '''
    {vbd_ref: virtual_device}
    eg: {'d2945e39-5f84-0fef-fbc4-13eb91c3354c': 'tda',
        'cc2945e39-5f84-0fef-fbc4-13eb91c3354' : 'tdd'
    }
    '''
    def dominfo_init_vbd2device(self, dev2path_list):
        
        dev2num_list = XendIOController.get_VM_dev2num(dev2path_list)
        vbd2num_list = {}
        vbd_refs = self.get_vbds()
        #log.debug('init_vbd2device...%s' % vbd_refs)
        for vbd_ref in vbd_refs:
            if self.dominfo_VBD_get(vbd_ref, 'type').lower() == 'disk':
                dev = self.dominfo_VBD_get(vbd_ref, 'device')
                if dev in dev2path_list.keys():
                    vbd2num_list[vbd_ref] = dev2num_list[dev]
        log.debug('dominfo_init_vbd2device: %s' % vbd2num_list)
        
        if vbd2num_list:
            major = vbd2num_list.values()[0]
            if major:
                major = major.split(":")[0]  # get the major device num
            #cmd = "ls -l /dev | grep '%s,' | awk 'gsub(/,/,\":\"){ print $5$6, $NF}'" % major
            cmd = "ls -l /dev/block | grep \"%s:\"| awk '{print $(NF-2), $NF}'" % major
            ...
            for line in out.split('\n'):
                ...
                if line:
                    ...
                    if result:
                        ...
                        num2device_list[num] = device
            #log.debug(num2device_list)
            
            vbd2device_list = {}
            for vbd_ref, num in vbd2num_list.items():
                if num in num2device_list:
                    vbd2device_list[vbd_ref] = num2device_list[num]
            #log.debug(vbd2device_list)
            return vbd2device_list
            
#     def dominfo_init_pid2devnum_list(self):
#         log.debug(">>>>>>>>dominfo_init_pid2devnum_list")
#         dev2path_list = self.dominfo_get_dev2path_list()
#         log.debug(dev2path_list)
#         dom_id = self.getDomid()
#         pid2devnum_list = XendIOController.get_VM_pid2num(dom_id, dev2path_list)
# #         log.debug('pid2devnum_list: %s' % pid2devnum_list)
#         vbd2device_list = self.dominfo_init_vbd2device(dev2path_list)
# #         log.debug('vbd2num_list: %s' % vbd2device_list)
#         self.info.setdefault('other_config',{})
#         self.info['other_config']['pid2dev'] = {}  #clear pid2dev_list          
#         for pid, devnum in pid2devnum_list.items():
#             self.info['other_config']['pid2dev'][pid] = devnum  
#         self.info['other_config']['vbd2device'] = {} # clear vbd2device_list
#         for vbd, device in vbd2device_list.items():
#             self.info['other_config']['vbd2device'][vbd] = device
#         log.debug('pid num to devnum: %s' % self.info['other_config']['pid2dev'])    
#         log.debug('vbd_uuid to vitual device name : %s ' % self.info['other_config']['vbd2device'])                     
  
    def dominfo_start_set_IO_limit(self):
        import threading
        t = threading.Thread(target=self._dominfo_start_set_IO_limit)
        t.setDaemon(True)
        t.start()
        
    
    def _dominfo_start_set_IO_limit(self):
        log.debug('in thread:start try to set_IO_limit')
        max_count = 0
        while True and max_count < 100:
            max_count += 1
            dom_id = self.getDomid()
            #log.debug('domid:%s' % dom_id)
            if dom_id and dom_id != '-1':
                break
            time.sleep(2)

        if not dom_id:
            log.exception('dominfo start set IO limit failed, dom id is None!')
            return
            
        max_count = 0
        while True and max_count < 150:
            max_count += 1
            dom_id = self.getDomid()
            if not dom_id or dom_id == '-1':
                log.exception('VM is no longer running, can not get pid.')
                break
            pid2dev_list = XendIOController.get_VM_pid2dev(dom_id)
            if pid2dev_list:
                break
            time.sleep(2)
        log.debug('get pid2dev_list: %s'% pid2dev_list)
        
#        self.dominfo_init_pid2devnum_list()
#        pid2num_list = self.info['other_config'].get('pid2dev',{})
        if not pid2dev_list:
            return
        dev2path_list = self._dominfo_get_dev2path_list()
        pid2num_list = XendIOController.get_VM_pid2num(dom_id, dev2path_list)
        for type in ['read', 'write']:
            for io_unit in ['MBps', 'iops']: 
                rate = self.dominfo_get_IO_rate_limit(type, io_unit)
                if rate != '-1':
                    self.dominfo_set_IO_rate_limit(pid2num_list, type, rate, io_unit)
        XendDomain.instance().managed_config_save(self)
    ...
    '''
    def _dominfo_get_dev2path_list(self):
        dev2path_list = {}
        vbd_refs = self.get_vbds()
        for vbd_ref in vbd_refs:
            if self.get_dev_property("vbd", vbd_ref, "type").lower() == 'disk':
                dev = self.get_dev_property("vbd", vbd_ref, "device")
#                 vdi_ref = self.get_dev_property("vbd", vbd_ref, "VDI")
#                 location = XendNode.instance().get_vdi_by_uuid(vdi_ref).location
                location = self.get_dev_property("vbd", vbd_ref, "uname")
    ...
    def dominfo_start_actions(self):
        log.debug('dominfo_start_action...')
        self.dominfo_set_all_tag() 
        self.dominfo_set_all_rate('rate')
        self.dominfo_set_all_rate('burst') 
        self.dominfo_start_set_IO_limit()
        
        
    def dominfo_clean_IO_limit_shutdown(self):
        import threading
        t = threading.Thread(target=self._dominfo_clean_IO_limit_shutdown)
        t.setDaemon(True)
        t.start()
        
    def _dominfo_clean_IO_limit_shutdown(self):
        log.debug('shutdown clean: pid2dev and rate limit in cgroup file')
        if self.info.get('other_config', {}) and \
         'pid2dev' in self.info['other_config']:
            pid2num_list = self.info['other_config']['pid2dev']
            for type in ['read', 'write']:
                for io_unit in ['MBps', 'iops']:
                    XendIOController.clear_VM_IO_rate_limit(pid2num_list, type, io_unit)
            del self.info['other_config']['pid2dev']                            
    ...
    def start(self, is_managed = False):
        ...
        if self._stateGet() in (XEN_API_VM_POWER_STATE_HALTED, XEN_API_VM_POWER_STATE_SUSPENDED, XEN_API_VM_POWER_STATE_CRASHED):
            try:
                self._check_serial_port_when_start()
                xendomains = XendDomain.instance()
                #check rbd mapping
                if xendomains._is_domain_use_rbd(self.get_uuid()):
                    xendomains._map_rbd_dev(self.get_uuid())
                ...
                if is_managed:
                    ...
                    self.dominfo_start_actions() #add by wufan 
            except:
    ...
    def shutdown(self, reason):
        ...
        if self._stateGet() in (DOM_STATE_SHUTDOWN, DOM_STATE_HALTED,):
            raise XendError(XEND_ERROR_VM_BAD_POWER_STATE)

        if self.domid == 0:
            raise XendError(XEND_ERROR_OPERATION_NOT_ALLOWED)
        ...
        if self.info.is_hvm():
            self.dominfo_clean_IO_limit_shutdown() #add by wufan
...
#             except VmError, ex:
    ...
    def getBlockDeviceClass(self, devid):
        ...
        if self._stateGet() in (DOM_STATE_RUNNING, DOM_STATE_PAUSED, DOM_STATE_CRASHED):
            ...
            # the domain is not active so we must get the device class
            # from the config
    ...
    def storeBackendDevice(self, backend_path, *args):
        return xstransact.Store(backend_path, *args)
    
    def _readBackendDevice(self, backend_path, *args):
        return xstransact.Read(backend_path, *args)
    ...
    def getDomainIp(self):
        ...
        if not domid or cmp(domid, -1) == 0:
            ...
            ipaddr = []
            domuuid = self.get_uuid()
            filename = '%s/ipaddr' % domuuid
            filepath = os.path.join(GUEST_DIR, filename)
            ...
            try:
                while 1:
                    ...
                    if not line:
                        break
                    ip_s = re.search('(\S+)$', line)
                    if ip_s:
                        ipaddr.append(ip_s.group(1))
            except Exception, exn:
                log.debug(exn)
                f.close()
                return ipaddr
...
#            log.debug(ipaddr)
    ...
    #frank
    def getDomainMAC(self):
        try:
            vifs = self.get_vifs()
            macs = []
            mac = None
            for vif in vifs:
                mac = self.get_dev_property('vif', vif, 'MAC')
                if mac:
                    macs.append(mac)
            return macs
        except Exception, exn:
            log.exception("Dom %s MAC exception!!!" % self.getName())
            log.exception(exn)
            return []
         
    #wufan   
    def set_vif_physical_network(self, vif_ref, phy_network ):
        log.debug('xendomainInfo VIF(%s)_set_physical_network on vm(%s)' % (vif_ref, self.get_uuid()))
        ...
        log.debug('in xenddomainInfo: %s' % network_names)
        if phy_network not in network_names:
            raise XendError('Network name do not exist!')
     
        xendom = XendDomain.instance()
        dom = xendom.get_vm_with_dev_uuid('vif', vif_ref)
        if not dom:
            log.debug('vif cannot be found on vm!')
            raise XendError('vif cannot be found on vm!')
 
        if dom._stateGet() == XEN_API_VM_POWER_STATE_RUNNING:
            log.debug('VM is running!')
            raise XendError('VM is running!')
        
        origin_network = xendom.get_dev_property_by_uuid('vif', vif_ref, 'network')
        new_network = network_names[phy_network]
        
        origin_bridge =  XendAPIStore.get(origin_network, "network").get_name_label()
...
#         log.debug('origin_bridge: %s and new_bridge: %s' % (origin_bridge, new_bridge))
        ...
        if cmp(origin_network, new_network) != 0 :
            rc = xendom.set_dev_property_by_uuid(
                       'vif', vif_ref, 'network', new_network, origin_network)
         
        if cmp(origin_bridge, new_bridge) != 0:
            rc1 = xendom.set_dev_property_by_uuid(
                         'vif', vif_ref, 'bridge', new_bridge, origin_bridge)                     
          
        if rc == False or rc1 == False:
            log.debug('set vif physical network failed')
            raise XendError('set vif physical network failed')   
    ...
    def setNameDescription(self, description, to_store = True):
        self.info['description'] = description
        if to_store:
            self.storeVm("description", description)
    ...
    def setVCpuCount(self, vcpus):
        def vcpus_valid(n):
            ...
            if vcpus > CPU_NUM_LIMIT:
                raise XendError('Cannot set vcpus greater than limit %s' % str(CPU_NUM_LIMIT))
        ...
        for v in vcpus:
            try:
                if self._stateGet() in (DOM_STATE_RUNNING, DOM_STATE_PAUSED):
                    rc = xc.vcpu_setaffinity(self.getDomid(), v, cpumap)
                cpus[v] = cpumap
            except Exception, ex:
                log.exception(ex)
    ...
    def getMemoryMaximumInMB(self):
        return self.info['memory_static_max'] / 1024 / 1024
    ...
    def getVCPUsAffinity(self):
        domid = self.getDomid()
        if domid is not None:
            params_live = {}
            vcpus_max = self.info['VCPUs_max']
...
#            return
        ...
        try:
            ...
            new_dom.dominfo_start_actions() #add by wufan 
...
#                           (" as domain %s" % str(dom.domid)) or ""))
    def _new_name(self, name):
        ...
        char_random = chr(a)
        return '%s_%s%s' % (name, char_random, str(now()))
    ...
    def set_is_a_template(self, is_template):
        self.info['is_a_template'] = is_template
        xen.xend.XendDomain.instance().managed_config_save(self)
        
#    #split ip map
#    def split_ip_map(self, ip_map):
#        retv = {}
#        if ip_map:
#            tmp = ip_map.split('@')
#            if len(tmp) > 1:
#                retv[tmp[0]] = tmp[1]
#        else:
#            log.debug("Ip map is empty!")
#        return retv
#    
#    #Get ip mapping.
#    def get_ip_map(self):
#        return self.info['other_config'].get('ip_map', {})
#    
#    def get_ip_map_value(self, mac):
#        return self.info['other_config'].get('ip_map', {}).get(mac, None)
#        
#    #Mapping intranet ip address to outer net ip address.
#    def set_ip_map(self, mac, del_map=False):
#        try:
#            oldval = self.get_ip_map()
#            if del_map:
#                value = self.get_ip_map_value(mac)
#                tmp = None
#                if value:
#                    tmp = self.split_ip_map(value)
#                if tmp:
#                    for intranet, outernet in tmp.items():
#                        Netctl.del_nat(intranet, outernet)
#                        Netctl.del_mac_bind(intranet, mac)
##                        Netctl.del_mac2ip(mac)
#                        Netctl.del_firewall_rule('tcp', intranet, '3389')
#                        Netctl.del_firewall_rule('tcp', intranet, '22')
#                        Netctl.firewall_deny_ping(intranet)
#                    if mac in oldval:
#                        del oldval[mac]
#                else:
#                    log.exception("Intranet ip not exists.")
#            else:
#                ip_map = Netctl.get_available_ip_map()
#                if not ip_map:
#                    raise XendError("No ip availed.")
#                tmp = self.split_ip_map(ip_map)
#                if tmp:
#                    log.debug("++++++ip_map+++++")
#                    log.debug(tmp)
#                    for intranet, outernet in tmp.items():
#        #                        available_intranet = Netctl.get_available_intranet()
#                        Netctl.add_mac_bind(intranet, mac)
#                        Netctl.add_nat(intranet, outernet)
##                        Netctl.insert_mac2ip(mac, intranet, outernet)
#                        Netctl.set_firewall_rule('tcp', intranet, '3389')
#                        Netctl.set_firewall_rule('tcp', intranet, '22')
#                        Netctl.firewall_allow_ping(intranet)
#                    if oldval:
#                        self.info['other_config']['ip_map'][mac] = '%s@%s' % (intranet, outernet)
#                    else:
#                        self.info['other_config']['ip_map'] = {mac : '%s@%s' % (intranet, outernet)}
#        except Exception, ex:
#            self.info['other_config']['ip_map'] = oldval
#            raise 
    
    # set passwd when vm start  
    def dominfo_set_passwd(self, vm_ip, passwd, origin_passwd = 'onceas'):
        log.debug('dominfo set passwd thread start>>>>>>>>>>>>')
        import threading
        t = threading.Thread(target=self._dominfo_set_passwd, args = (vm_ip, passwd, origin_passwd))
        t.setDaemon(True)
        t.start()
       
    # try to change passwd, time limit 240s 
    def _dominfo_set_passwd(self, vm_ip, passwd, origin_passwd = 'onceas'):
        from xen.xend import ssh, encoding
        count = 0
        flag = False
        while True and count < 240:
            cmd = 'echo "%s" | passwd --stdin root' % passwd
            result = ssh.ssh_cmd2(vm_ip, cmd, origin_passwd)
            log.debug(cmd)
            log.debug(result)
            if  result:
                flag = True
            ...
            time.sleep(1)
        if not flag:
            log.debug('set vm: %s passwd failed!' % vm_ip)
        
    
     
     
    def set_rate(self, type, eth_num, value):
        if type != 'rate' and type != 'burst':
            return
        domid = self.getDomid()
        if not domid or domid == '-1':
            return 
        if "-" in value:
            log.debug('value %s invalid!' % value)
            return 
#        cmd = '/usr/bin/ovs-vsctl set Interface tap%s.%s ingress_policing_%s=%s' %(str(domid), str(eth_num), type, value)
        cmd1 = '%s set Interface vif%s.%s ingress_policing_%s=%s' %(OVS_VSCTL, str(domid), str(eth_num), type, value)
#        log.debug(cmd)
        log.debug(cmd1)
#        (rc, stdout, stderr) = doexec(cmd)
#        if rc != 0:
...
#            stdout.close();
#            stderr.close();
#            log.exception('Failed to set vm(%s) tap %s: %s.%s' % (self.getName(), type, err, str(eth_num)))
#        stdout.close()
#        stderr.close()
        ...
        if rc != 0:
            ...
            stdout.close();
            stderr.close();
            log.exception('Failed to set vm(%s) vif %s: %s.%s' % (self.getName(), type, err, str(eth_num)))
        stdout.close()
        stderr.close()
        
    def set_tag(self, eth_num, value):
        domid = self.getDomid()
        if not domid or domid == '-1':
            return 
        if cmp(value, "-1") == 0:
#            cmd = '/usr/bin/ovs-vsctl clear port tap%s.%s tag' %(str(domid), str(eth_num))
            cmd1 = '%s clear port vif%s.%s tag' %(OVS_VSCTL, str(domid), str(eth_num))
        else:
#            cmd = '/usr/bin/ovs-vsctl set port tap%s.%s tag=%s' %(str(domid), str(eth_num), value)
            cmd1 = '%s set port vif%s.%s tag=%s' %(OVS_VSCTL, str(domid), str(eth_num), value)
#        log.debug(cmd)
        log.debug(cmd1)
#        (rc, stdout, stderr) = doexec(cmd)
#        if rc != 0:
...
#            stdout.close();
#            stderr.close();
#            log.exception('Failed to set vm(%s) tap tag: %s.%s' % (self.getName(), err, str(eth_num)))
#        stdout.close()
#        stderr.close()
        (rc, stdout, stderr) = doexec(cmd1)
        if rc != 0:
            ...
            stdout.close();
            stderr.close();
            log.exception('Failed to set vm(%s) vif tag: %s.%s' % (self.getName(), err, str(eth_num)))
        stdout.close()
        stderr.close()

    def switch_vif_to_different_ovs_and_set_tag(self, eth_num, value, ovs, bridge):
        domid = self.getDomid()
        if not domid or domid == '-1':
            return 
        cmd1 = '%s del-port %s vif%s.%s' % (OVS_VSCTL, bridge, str(domid), str(eth_num))
        log.debug(cmd1)
        (rc, stdout, stderr) = doexec_timeout(cmd1, 5)
        if rc == None:
            raise Exception, 'switch_vif_to_different_ovs_and_set_tag timeout!'
        if rc != 0:
            err = stderr.read();
            out = stdout.read();
            stdout.close();
            stderr.close();
            raise Exception, 'Failed to set vm(%s) vif tag: %s.' % (self.getName(), err)
        stdout.close()
        stderr.close()
        cmd2 = '%s add-port %s vif%s.%s' % (OVS_VSCTL, ovs, str(domid), str(eth_num))
        log.debug(cmd2)
        ...
        stderr.close()
        if cmp(value, "-1") == 0:
            cmd3 = '%s clear port vif%s.%s tag' %(OVS_VSCTL, str(domid), str(eth_num))
        else:
            cmd3 = '%s set port vif%s.%s tag=%s' %(OVS_VSCTL, str(domid), str(eth_num), value)
        log.debug(cmd3)
        (rc, stdout, stderr) = doexec_timeout(cmd3, 5)
        if rc == None:
            raise Exception, 'switch_vif_to_different_ovs_and_set_tag timeout!'
        if rc != 0:
            err = stderr.read();
...
#     '''
#     check whether vif is in used
#     '''
#     def check_vif_in_use(self, vif_name):
#         in_use = True
#         cmd = 'ifconfig | grep "^%s\s"' % vif_name
#         log.debug('check vif in use: excute cmd: %s' % cmd)
#         (rc, stdout, stderr) = doexec(cmd)
#         if rc != 0: # vif if not in use
#             log.debug('vif(%s) is not in use......')
#             in_use = False
#         stdout.close()
#         stderr.close()
#         return in_use
#         
#         
#     def check_vif_create(self, vif_name):
#         is_created = True
#         cmd = 'ip addr | grep "\b%s:\s"' % vif_name
#         log.debug('check vif create: excute cmd: %s' % cmd)
#         (rc, stdout, stderr) = doexec(cmd)
#         if rc != 0: # vif if not in use
#             log.debug('vif(%s) is not in use......')
#             is_created = False
...
#         return is_created
#     
#     def vif_make_in_use(self, vif_name):
#         cmd = 'ifconfig %s up' % vif_name
#         log.debug('vif_make_in_use: excute cmd: %s' % cmd)
#         (rc, stdout, stderr) = doexec(cmd)
#        
#         if rc != 0:
#             err = stderr.read();
...
#             stdout.close()
#             stderr.close()
#             log.debug('vif make in use error>>>>>%s' % err)
#             return False
#         else:
#             cmd = '/usr/bin/ovs-vsctl add-port ovs0 %s' % vif_name
...
#             
    ...
    '''
    for each vif: check it has been created and make in use
    '''
    def check_vif_and_make_in_use(self, vif_name):
        in_use = self.check_vif_in_use(vif_name)
        if not in_use:
            is_created = self.check_vif_create(vif_name)  
            if is_created:
                pass
    ...
    def set_memory_static_max(self, val):
        if not val < MEM_LIMIT * GB:
            raise XendError("memory_static_max must be less " \
                                  "than %s GB, %s(%s) is the value" % (str(MEM_LIMIT), str(self["memory_static_max"]), self['name_label']))        
    ...
    #frank
    def get_cpu_qos(self):
        return self.cpu_qos.get_uuid();
    #frank
    def get_network_qos(self):
        return self.network_qos.get_uuid();
...
#                 rd_blkps = 0.0
#                 wr_blkps = 0.0
#                 if self.info.get('other_config', '') and self.info['other_config'].get('vbd2device'):  
#                     device_name = self.info['other_config']['vbd2device'].get(dev_uuid, '')
#                     if device_name:
#                         rd_blkps, wr_blkps = xennode.get_vbd_iorate(self.domid, device_name)
#                     else:
#                         rd_blkps, wr_blkps = xennode.get_vbd_util(self.domid, dev_uuid)
#                 else:
#                     rd_blkps, wr_blkps = xennode.get_vbd_util(self.domid, dev_uuid)
            ...
            VDI_ref = config.get('VDI', '')   
            if not VDI_ref:
                vdi_path = config.get('uname', '')
                if vdi_path:
                    pat = '/(\w+-\w+-\w+-\w+-\w+)/'
                    m = re.search(pat, vdi_path)
                    if m:
                        VDI_ref = m.group(1)
#                         log.debug('VDI ref: %s' % VDI_ref)
            ...
            if ':' in config['device']:
                ...
                if config.get('type', '') in XEN_API_VBD_TYPE:
        ...
        if dev_class == 'console':
            if not config.has_key('protocol'):
                config['protocol'] = config.get('protocol', 'UNKNOWN')
            if not config.has_key('location'):
                config['location'] = config.get('location', '')
            if not config.has_key('other_config'):
                config['other_config'] = config.get('other_config', {})
    ...
    def set_dev_property(self, dev_class, dev_uuid, field, value, to_store = True):
        self.info['devices'][dev_uuid][1][field] = value
        domid = self.getDomid()
        if domid:
            dev_backend_id = self.get_dev_property(dev_class, dev_uuid, "id")
            backend_path = "/local/domain/0/backend/%s/%s/%s" % (dev_class, domid, dev_backend_id)
            log.debug("backend path: %s" % backend_path)
            self.storeBackendDevice(backend_path, field, value)
    ...
    def get_console_port(self):
        return "0.0.0.0:%s" % self.readDom('console/vnc-port')
#         cmd = "cat /var/lib/xend/domains/%s/config.sxp | grep location" % self.get_uuid()
...
#         stderr.close()        
    ...
    def create_vbd_for_xenapi(self, xenapi_vbd, vdi_image_path):
        ...
        try:
            if not xenapi_vbd.get('device', ''):
                avalid_device = self._get_available_vbd_device('hd')
                if not avalid_device:
                    raise XendError('No avaliable device.')
                else:
                    xenapi_vbd['device'] = avalid_device
            else:
                is_device_avaliable = self._is_vbd_device_available(xenapi_vbd.get('device'))
                if not is_device_avaliable:
                    raise XendError('INVALID_VBD_DEVICE')
            current_VDIs = self._get_current_VDIs()
            if xenapi_vbd.get('VDI', '') in current_VDIs:
                raise XendError('Invalid binding VDI, already binding: %s' % xenapi_vbd.get('VDI', ''))
            if cmp(xenapi_vbd.get('type', ''), 'Disk') == 0:
                system_vdi_driver = self._get_system_vbd_driver()
                if cmp(system_vdi_driver, vdi_image_path.split(':')[0]) != 0:
                    if cmp(system_vdi_driver, 'tap') == 0:
                        vdi_image_path = 'tap:aio:%s' % vdi_image_path.split(':')[-1]
                    elif cmp(system_vdi_driver, 'phy') == 0:
                        vdi_image_path = 'phy:%s' % vdi_image_path.split(':')[-1]
                    elif cmp(system_vdi_driver, 'file') == 0:
                        vdi_image_path = 'file:%s' % vdi_image_path.split(':')[-1]
...
#            self.__vbd_lock__.release()
    ...
    def _get_available_vbd_device(self, type): 
        vbds = self.get_vbds()
        if vbds:
            if cmp(len(vbds), DISK_LIMIT+1) >=0:
                log.exception("Can not create new disk, disk limit is: %s!" % DISK_LIMIT)
                return None
            device = self.get_dev_property('vbd', vbds[0], 'device')
            if device and cmp(device[:2], "hd") == 0:
                device_list = copy.deepcopy(VBD_DEFAULT_DEVICE)
            elif device:
                device_list = copy.deepcopy(VBD_XEN_DEFAULT_DEVICE)
            else:
                log.exception("VBD device not found: %s" % vbds[0])
            for vbd in vbds:
                device = self.get_dev_property('vbd', vbd, 'device')
                if device and device in device_list:
                    device_list.remove(device)
                elif device:
                    continue
                else:
                    log.exception("VBD device not found: %s" % vbd)
            if device_list:
            ...
            if cmp(type, 'hd') == 0:
                return VBD_DEFAULT_DEVICE[0]
            else:
                return VBD_XEN_DEFAULT_DEVICE[0]
        
    def _is_vbd_device_available(self, device):
        if device not in VBD_DEFAULT_DEVICE and device not in VBD_XEN_DEFAULT_DEVICE:
            return False
        vbds = self.get_vbds()
        if vbds:
            for vbd in vbds:
                exist_device = self.get_dev_property('vbd', vbd, 'device')
                if exist_device and cmp(exist_device, device) == 0:
                    return False
                elif exist_device and exist_device[:2] not in device:
                    return False
        return True                       
    
    def _get_current_VDIs(self):
        try:
            vbds = self.get_vbds()
            vdis = []
            for vbd in vbds:
                vdi = self.get_dev_property('vbd', vbd, 'VDI')
                vdis.append(vdi)
            return vdis
        except Exception, exn:
            log.error(exn)
    ...
    def create_console(self, xenapi_console):
        ...
        if self.get_consoles():
            raise XendError('Failed to create conflict console')
    ...
    def create_dpci(self, xenapi_pci):
        ...
        dpci_uuid = uuid.gen_regularUuid()
    ...
    def create_dscsi(self, xenapi_dscsi):
        ...
        dscsi_uuid = uuid.gen_regularUuid()
    ...
    def create_dscsi_HBA(self, xenapi_dscsi):
        ...
        dscsi_HBA_uuid = uuid.gen_regularUuid()
    ...
    def _check_serial_port_when_start(self):
        plat = self.get_platform()
        avail_port = self.get_free_port()
        serial_value = 'tcp:127.0.0.1:%s,server,nowait' % avail_port
        plat['serial'] = serial_value
        self.info['platform'] = plat
        xen.xend.XendDomain.instance().managed_config_save(self)
    
    #get free port from 14000 to 15000 for VM serial tcp server.        
    def get_free_port(self):
        (start_port, end_port) = PORTS_FOR_SERIAL
        for i in xrange(start_port, end_port):
            if not self._is_port_in_use(i):
                return i
            else:
                continue
        raise XendError('No port available! From %s to %s.' % (start_port, end_port))
        
    def _is_port_in_use(self, port):
        cmd = 'netstat -tulnp | grep -c %s' % port
        (rc, stdout, stderr) = doexec_timeout(cmd, 5)
#        for line in os.popen(cmd):
        if rc == None:
#            log.exception('netstat -tulnp | grep %s, timeout!' % port)
            raise Exception('netstat -tulnp | grep -c %s. timeout!' % port)
        else:
            out = stdout.read();
            stdout.close();
            if out.strip() != "0":
    ...
    def destroy_device_by_uuid(self, dev_type, dev_uuid):
        self.__device_delete_lock__.acquire()
        try:
            if dev_uuid not in self.info['devices']:
                raise XendError('Device does not exist')
            if self._stateGet() in (XEN_API_VM_POWER_STATE_RUNNING,
                ...
                if not devid:
                    devid = dev_name_to_number(config.get('dev', ''))
#                log.debug(dev_type)
#                log.debug(config)
                if not devid and dev_type == 'vif':
                    devid = self._get_vif_devid_by_eth_num(dev_uuid)
                log.debug("devid: %s" % str(devid))
                if devid != None:
            ...
            self.__device_delete_lock__.release()
            
    def _get_vif_devid_by_eth_num(self, dev_uuid):
        device = self.get_dev_property('vif', dev_uuid, 'device')
        retv = None
        if device:
            retv = device[3:]
        return retv
    ...
    def destroy_vif(self, dev_uuid):
        devid = self._get_vif_devid_by_eth_num(dev_uuid)
        if devid:
            self.destroyDevice('vif', devid, True, True)
        else:
            self.destroy_device_by_uuid('vif', dev_uuid)

    #frank
    def destroy_console(self, dev_uuid):
        self.destroy_device_by_uuid('console', dev_uuid)
    ...
    def destroy_xapi_instances(self):
        ...
        # Destroy the VMMetrics instance.
        if XendAPIStore.get(self.cpu_qos.get_uuid(), self.cpu_qos.getClass()) \
                is not None:
            self.cpu_qos.destroy()
