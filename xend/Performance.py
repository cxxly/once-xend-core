...
class Performance:
    ...
    def collect_vms(self):
        ...
        self.pif_read_total = 0.0
        self.pif_write_total = 0.0
        ...
        for vm in self.vms:
            ...
            for vif in self.domain.get_vifs(vm):
                ...
                self.pif_read_total += vif_record['io_read_kbs']
                self.pif_write_total += vif_record['io_write_kbs']
    ...
    def write(self):
        ...
        # ugly hack for adding ovs0 metrics
        if not self.host_pifs:
            host_metrics.append("pif_0_rx:" + self.format(self.pif_read_total))
            host_metrics.append("pif_0_tx:" + self.format(self.pif_write_total))
...
import threading
import time
class RunPerformance(threading.Thread):
