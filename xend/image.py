...
from XendLogging import log_image, init
...
class HVMImageHandler(ImageHandler):
    ...
    def parseDeviceModelArgs(self, vmConfig):
        ...
        if not enable_qemu_xen:
            ...
            ret = ret + ['-smp' , '%s,maxcpus=%s' % (str(self.vm.getVCpuCount()), str(self.vm.getVCpuCount()))]
            ret = ret + ['-m' , str(self.vm.getMemoryMaximumInMB())]
                ...
                if v:
                    ...
                    for s in v:
                        if s:
                            ret.append("-serial")
                            ret.append("%s" % s)
            elif a == 'boot' and enable_qemu_xen:
                if v:
                    ret.append("-%s" % a)
                    ret.append("order=%s" % v)
            else:
                if v:
                    ret.append("-%s" % a)
                    ret.append("%s" % v)
            ...
            if uname is not None and 'file:' in uname:
                ...
                if not os.path.isfile(vbdparam):
                    raise VmError('Disk image does not exist: %s' %
                                  vbdparam)
            if uname is not None and cmp(devinfo.get('mode'), 'w') == 0 and \
                cmp(devinfo.get('bootable'), 1) == 0 and enable_qemu_xen:
                vbd_path = uname.split(':')[-1]
                log.debug('System VBD path: %s' % vbd_path)
                ret.append("-drive")
                ret.append("file=%s,if=ide,index=0,media=disk,format=raw,cache=writeback" % vbd_path)
...
#             args = args + ([ "-machine", "pc-i440fx-2.0,accel=xen"])
#             args = args + (["-global", "PIIX4_PM.acpi-pci-hotplug-with-bridge-support=off"])
#             args = args + (["-device", "xen-platform,addr=0x2"])
            args = args + (["-device", "VGA"])
