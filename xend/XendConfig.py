...
from xen.xend.XendConstants import DOM_STATE_HALTED, AUTO_PHP_SLOT, XEND_CONFIG_FILE
...
def dev_name_to_number(dev):
    ...
    try:
        ...
        if dev2 == None:
            log.warning("Could not check the device %s", dev)
            return None
        try:
            devid = int(dev2)
        except ValueError:
            ...
            if devid == None:
                log.warning("The device %s is not device name", dev2)
                return None
    return devid
...
class XendConfig(dict):
        ...
        for event in ['shutdown', 'reboot', 'crash']:
            if self['actions_after_' + event] not in CONFIG_RESTART_MODES:
                raise XendConfigError('Invalid event handling mode: ' + 
                                      event + ' ' + self['actions_after_' + event])
    ...
    def _dominfo_to_xapi(self, dominfo, update_mem=False):
        ...
        if update_mem:
            if dominfo['mem_kb'] <= 0:
                dominfo['mem_kb'] = dominfo['maxmem_kb']
...
#        f.close()
    ...
    def _parse_sxp(self, sxp_cfg):
        ...
        for key, typ in XENAPI_CFG_TYPES.items():
            ...
            if val is not None:
                try:
                    ...
                    if key == 'other_config':
                        cfg[key] = {}
                        for k, v in typ(val).items():
                            if k in ('tag', 'rate', 'burst', 'pid2dev', 'ip_map', 'vbd2device'):
                                cfg[key][k] = dict(v)
                            else:
                                cfg[key][k] = v
                    else:    
        ...
        #log.debug(' return add device:+++++++++++ ')
        #log.debug(cfg['devices'])
        return cfg
    ...
    def _sxp_to_xapi(self, sxp_cfg):
        ...
        # beforehand.
        self['uuid'] = sxp.child_value(sxp_cfg, 'uuid', uuid.gen_regularUuid())
    ...
    def update_with_xenapi_config(self, xapi):
        ...
        cpus = []
        if 'cpus' not in xapi or not xapi['cpus'][0]:
            self['cpus'] = ''
            for v in range(0, int(xapi['VCPUs_max'])):
                self['cpus'].append(cpus)
        elif isinstance(xapi['cpus'], list):
            self['cpus'] = xapi['cpus']
        else:
            self['cpus'] = self._convert_cpus_to_list(xapi['cpus'])

    def cpuid_to_sxp(self, sxpr, field):
        regs_list = []
        for input, regs in self[field].items():
            reg_list = []
    ...
    def otherconfig_tag_to_sxp(self, sxpr):
        othcfg_list = []
        for k_cfg, v_cfg in self['other_config'].iteritems():
            if k_cfg in ('tag', 'rate', 'burst', 'pid2dev', 'ip_map', 'vbd2device'):
                othcfg_list.append([k_cfg, v_cfg.items()])
            else:
                othcfg_list.append([k_cfg, v_cfg])
        sxpr.append(['other_config', othcfg_list])
            ...
            for name, typ in XENAPI_CFG_TYPES.items():
                if name in self and self[name] not in (None, []):
                    ...
                    if name == 'other_config':
                        self.otherconfig_tag_to_sxp( sxpr) 
                    #add by wufan=====   
                    else:                                     
                        if typ == dict:
                            s = self[name].items()
                        ...
                        try:
                            ...
                            for config in configs:
                                if sxp.name(config) in ('vbd', 'tap', 'tap2'):
                                    ...
                                    if sxp.child_value(config, 'type', None) is None:
                                        type_fiber = dev_cfg.get('type', None)
                                        config.append(['type', str(type_fiber)])
                                    config.append(['VDI', dev_cfg.get('VDI', '')])

                                sxpr.append(['device', config])
                                if sxp.name(config) in ('vif', 'vif2'):
                                    dev_uuid = sxp.child_value(config, 'uuid')
                                    dev_type, dev_cfg = self['devices'][dev_uuid]
                                    bridge_old = sxp.child_value(config, 'bridge', None)
                                    bridge_new = dev_cfg.get('bridge', None)
                                    if cmp(bridge_old, bridge_new) != 0:
                                        config.remove(['bridge', bridge_old])
                                        config.append(['bridge', bridge_new])
#                                     log.warning(config)
#                                     log.warning(dev_cfg)
#                                     log.warning(sxp.child_value(config, 'bridge', None))
#                                     log.warning(sxp.child_value(config, 'network', None))
                                found = True
        ...
        if cfg_xenapi:
            ...
            if dev_type == 'vif':
                ...
                if cfg_xenapi.get('network'):
                    ...
                    if cfg_xenapi.get('network', ''):
                        dev_info['bridge'] = cfg_xenapi.get('network')
                    else:
                        dev_info['bridge'] = 'ovs0'#network.get_name_label()

                if cfg_xenapi.get('security_label'):
                    dev_info['security_label'] = \
                         cfg_xenapi.get('security_label')
                         
                if cfg_xenapi.get('ip'):
                    dev_info['ip'] = \
                        cfg_xenapi.get('ip')
                ...
                dev_info['uuid'] = dev_uuid
                #frank
                if not cfg_xenapi.get('script'):
                    dev_info['script'] = auxbin.pathToScript("vif-bridge")
                target['devices'][dev_uuid] = (dev_type, dev_info)
...
#                dev_info['driver'] = cfg_xenapi.get('driver', 'paravirtualised')
                ...
                dev_info['location'] = cfg_xenapi.get('location', '')
                ...
                if dev_info['protocol'] == 'rfb':
                    ...
                    dev_info['vnc'] = console_other_config.get('vnc', '1')
                    dev_info['sdl'] = console_other_config.get('sdl', '0')
                    dev_info['keymap'] = console_other_config.get('keymap', 'en-us')
                    dev_info['vncunused'] = console_other_config.get('vncunused', '1')
                    ...
                    vkbd_uuid = uuid.gen_regularUuid()
                    target['devices'][vkbd_uuid] = ('vkbd', {'uuid':vkbd_uuid})
                    ...
                    raise XendConfigError('Creating consoles with unsupported protocol: %s.' % dev_info['protocol'])
    ...
    def phantom_device_add(self, dev_type, cfg_xenapi=None,
        ...
        if cfg_xenapi:
            ...
            if dev_type in ('vbd', 'tap'):
                ...
                dev_info['backend'] = '0'
                dev_uuid = cfg_xenapi.get('uuid', uuid.gen_regularUuid())
                dev_info['uuid'] = dev_uuid
    ...
    def vscsi_convert_sxp_to_dict(self, dev_sxp):
        ...
        for vscsi_dev in sxp.children(dev_sxp, 'dev'):
            ...
            vscsi_uuid = vscsi_dev_info.get('uuid', uuid.gen_regularUuid())
    ...
    def device_update(self, dev_uuid, cfg_sxp=[], cfg_xenapi={}):
        ...
        if dev_uuid in self['devices'] and cfg_sxp:
            ...
            if dev_type == 'vbd' or dev_type.startswith('tap'):
                dev_type = config[0]
                log.debug(dev_type)
