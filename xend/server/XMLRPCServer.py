...
from xen.xend import XendAPI, XendDomain, XendDomainInfo, XendNode, BNPoolAPI, BNHostAPI, BNVMAPI, BNStorageAPI
...
class XMLRPCServer:
    def __init__(self, auth, use_xenapi, use_tcp = False,
        ...
        self.poolapi = use_xenapi and BNPoolAPI.BNPoolAPI(auth) or None
        self.hostapi = use_xenapi and BNHostAPI.BNHostAPI(auth) or None        
        self.vmapi = use_xenapi and BNVMAPI.BNVMAPI(auth) or None
        self.storageapi = use_xenapi and BNStorageAPI.BNStorageAPI(auth) or None
    def run(self):
...
#                 P_DataCollect.main()
        ...
        for meth_name in dir(self.poolapi):
            if meth_name[0] != '_':
                meth = getattr(self.poolapi, meth_name)
                if callable(meth) and hasattr(meth, 'api'):
                    self.server.register_function(meth, getattr(meth, 'api'))          
                    
        for meth_name in dir(self.hostapi):
            if meth_name[0] != '_':
                meth = getattr(self.hostapi, meth_name)
                if callable(meth) and hasattr(meth, 'api'):
                    self.server.register_function(meth, getattr(meth, 'api'))     
                    
        for meth_name in dir(self.vmapi):
            if meth_name[0] != '_':
                meth = getattr(self.vmapi, meth_name)
                if callable(meth) and hasattr(meth, 'api'):
                    self.server.register_function(meth, getattr(meth, 'api'))      
                    
        for meth_name in dir(self.storageapi):
            if meth_name[0] != '_':
                meth = getattr(self.storageapi, meth_name)
                if callable(meth) and hasattr(meth, 'api'):
                    self.server.register_function(meth, getattr(meth, 'api')) 
        self.server.register_instance(XendAPI.XendAPIAsyncProxy(self.xenapi))
        self.server.register_instance(BNPoolAPI.BNPoolAPIAsyncProxy(self.poolapi))
        self.server.register_instance(BNHostAPI.BNHostAPIAsyncProxy(self.hostapi))
        self.server.register_instance(BNVMAPI.BNVMAPIAsyncProxy(self.vmapi))
        self.server.register_instance(BNStorageAPI.BNStorageAPIAsyncProxy(self.storageapi))
