...
CMD_ROUTE    = 'route'
CMD_BRCTL    = os.popen("which ovs-vsctl").read().strip()
...
bridgeRE = re.compile('(\S+)$')
ifaceRE = re.compile('(\S+)$')

def get_state():
    fin = os.popen(CMD_BRCTL + ' list-br', 'r')
    try:
        bridges = {}
        for line in fin:
            line = line.strip('\n')
...
def get_ifaces(br = "ovs0"):
    fin = os.popen(CMD_BRCTL + ' list-ports %s' % br, 'r')
    try:
        brlist = []
        for line in fin:
            if line[0] == '\t':
...
#                brlist.append(line.strip())
                ...
                m = ifaceRE.search(line)
                brlist.append(m.group(1).strip())
        return brlist
    finally:
        fin.close()    
...
def vif_bridge_add(params):
    ...
    """
    cmd(CMD_BRCTL, 'add-port %(bridge)s %(vif)s' % params)

def vif_bridge_rem(params):
    """Remove the network interface for vif on dom from a bridge.
    """
    cmd(CMD_BRCTL, 'del-port %(bridge)s %(vif)s' % params)
