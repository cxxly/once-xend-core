...
SUBCOMMAND_HELP = {
    ...
    'set-vif-physical-network' : ('<domain><vif_ref><physical network>', 'Set the physical network of a vif'),
...
#frank     
def xm_domuuid_from_mac(args):
    arg_check(args, "domuuid", 1)   
    mac = args[0]
    if serverType == SERVER_XEN_API:
#        print 'yes'
        doms = server.xenapi.VM.get_all()
        for dom in doms:
            vif_refs = server.xenapi.VM.get_VIFs(dom)
            for vif_ref in vif_refs:
                if cmp (mac, server.xenapi.VIF.get_MAC(vif_ref)) == 0:
                    print server.xenapi.VM.get_domid(dom)
                else:
                    print 'not found'
    else:
        doms = getDomains(None, 'running')
        for dom in doms:
            domuuid = sxp.child_value(dom, "uuid")
            if cmp(domuuid, "00000000-0000-0000-0000-000000000000") == 0:
                continue
            devs = server.xend.domain.getDeviceSxprs(domuuid, 'vif')
            for dev in devs:
#                print sxp.child_value(dev[1], 'mac')
                if cmp(sxp.child_value(dev[1], 'mac'), mac) == 0:
                    print domuuid
...
# wufan     
def xm_set_vif_physical_network(args):
    arg_check(args, "set physical network", 3)  
    dom, vif_ref, phy_network = args

    if serverType == SERVER_XEN_API:
        server.xenapi.VIF.set_physical_network_local(get_single_vm(dom), vif_ref, phy_network)
    else:
        server.xend.domain.set_vif_physical_network(dom, vif_ref, phy_network)
...
commands = {
    ...
    "destroy": xm_destroy,
    "domid": xm_domid,
    "domuuid-from-mac" : xm_domuuid_from_mac,
    "set-vif-physical-network" : xm_set_vif_physical_network,
    "domname": xm_domname,
